...
export class MfaController {
  ...
  @ApiResponse({
    status: 201,
    description: 'Secret and authenticator URI. Display the secret only during setup.',
  })
  setup(@Body() dto: MfaSetupDto, @Req() request: AuthenticatedRequest) {
    return this.mfaService.setup(request.user.sub, dto.password);
  }
  ...
  async verifySetup(
    ...
    await this.mfaService.verifySetup(request.user.sub, dto.password, dto.code);
  }
